...
static boolean P_CrossSubsector (int num, register los_t *los)
{
    ...
    if (num >= numsubsectors)
    {
        I_Error(english_language ? 
                ...
                "P_CrossSubsector: ss %i á numss = %i", num, numsubsectors);
    }
#endif
    ...
    for (count = subsectors[num].numlines; --count >= 0; seg++)  // check lines
    {
        ...
        // [JN] A "singleplayer" condition works fine, but let's just don't use it.
        /*
        if (singleplayer)
...
static boolean P_CrossBSPNode (int bspnum, register los_t *los) 
{
    while (!(bspnum & NF_SUBSECTOR))
    {
        ...
        else if (!P_CrossBSPNode(bsp->children[side], los))
        {
...
boolean P_CheckSight (mobj_t *t1, mobj_t *t2)
{
    ...
    // [JN] A "singleplayer" condition works fine, but let's just don't use it.
    /*
    if ((t1->subsector == t2->subsector) && singleplayer)
    return true;
    */
