    ...
    do
    {
        if (rover == start)
        {
            ...
            I_Error (english_language ?
                     ...
                     "Z_Malloc: Ошибка обнаружения %i байт памяти", size);
        }
...
void Z_DumpHeap (int lowtag, int hightag)
{
    ...
    for (block = mainzone->blocklist.next ; ; block = block->next)
    {
        if (block->tag >= lowtag && block->tag <= hightag)
        {
            ...
            "блок:%p    размер:%7i    пользователь:%p    тэг:%3i\n",
        ...
        if ((byte *)block + block->size != (byte *)block->next)
        {
            printf (english_language ?
                    ...
                    "ОШИБКА: размер блока не достигает следующего блока\n");
        }
        ...
        if (!block->user && !block->next->user)
        {
            printf (english_language ?
                    ...
                    "ОШИБКА: освобождено два блока подряд\n");
        }
    }
}
    ...
    for (block = mainzone->blocklist.next ; ; block = block->next)
    {
        ...
        if ((byte *)block + block->size != (byte *)block->next)
        {
            I_Error (english_language ?
                     ...
                     "Z_CheckHeap: размер блока не затрагивает следующий блок\n");
        }
        ...
        if (block->next->prev != block)
        {
            I_Error (english_language ?
                     ...
                     "Z_CheckHeap: следующий блок не имеет корректной обратной связи\n");
        }
        ...
        if (!block->user && !block->next->user)
        {
            I_Error (english_language ?
                     ...
                     "Z_CheckHeap: обнаружены два последовательных свободных блока\n");
        }
    }
}
...
void Z_ChangeTag2 (void *ptr, int tag)
{
    ...
    if (block->id != ZONEID)
    {
        I_Error (english_language ?
                 ...
                 "Z_ChangeTag: высвобождение указателя без ZONEID");
    }
    ...
    if (tag >= PU_PURGELEVEL && (unsigned)block->user < 0x100)
    {
        I_Error (english_language ?
                 ...
                 "Z_ChangeTag: для очищаемых блоков памяти требуется административный объект");
    }
