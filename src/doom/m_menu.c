...
//
// Описание:
// * Поправлено расположение иконки M_SKULL для Final DOOM.
// * Окнам сохранения, загрузки и громкости присвоены дополнительные
//   заголовки из прописных букв. Поля ввода и ползунков 
//   отцентрированы на экране.
// * Предел чувствительности мыши увеличен с 9 до 20, расширена 
//   полоса ползунка в меню настроек игры. Кроме того, игра более
//   не будет аварийно завершаться, если в конфигурационном файле
//   указаны слишком высокие значения, при которых ползунок мог 
//   оказаться за краем экрана. В такой ситуации ползунок поменяет 
//   цвет с синего на красный, и вернёт синий цвет только когда 
//   вернётся к допустимым значениям. Спасибо Фабиану Греффрату!
// =--------------------------------------------------------------=
...
// [JN] Размер экрана по-умолчанию установлен на 10
int			detailLevel = 0;
int			screenblocks = 10;
  ...
  int	thermDot )
{
    ...
    V_DrawPatchDirect((x + 8) + thermDot * 8, y,
		      W_CacheLumpName(DEH_String("M_THERMW"), PU_CACHE));
    }
    // [JN] А если ползунок в крайнем левом положении, делать его затемненным
    else if (thermDot == 0)
    {
    V_DrawPatchDirect((x + 8) + thermDot * 8, y,
		      W_CacheLumpName(DEH_String("M_THERMD"), PU_CACHE));
    }
    else
    {
    V_DrawPatchDirect((x + 8) + thermDot * 8, y,
