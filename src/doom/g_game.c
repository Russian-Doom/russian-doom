...
//
// Описание:
// * Переведена строчка "V_ScreenShot", отображающая сообщение 
//   "Снимок экрана" в режиме разработчика.
// * Переведена строчка "Режим проверки управления".
// * Отключен баг "Sky never changes" для Doom II.
// * Переведена строчка "is turbo!".
// * Добавлен твик, при котором в режиме "Всегда бежать" и нажатии
//   кнопки бега игрок переходит на шаг. 
//   По методу Фабиана Греффрата.
// * Добавлена возможность переключаться на кулак при наличии
//   бензопилы.
// * Добавлено рекордное время для четвёртого эпизода из BFG Edition
// * Исправлен баг со скоростью демона при сохранении и загрузке 
//   в обычном и кошмарных режимах.
// =--------------------------------------------------------------=
...
//         || joybuttons[joybspeed];
    ...
    if (gamekeydown[key_toggleautorun])
    {
        ...
        M_snprintf(autorunmsg, sizeof(autorunmsg), "gjcnjzyysq ,tu %s", // ALWAYS RUN %s
            (joybspeed >= MAX_JOY_BUTTONS) ? "drk.xty" : "dsrk.xty"); // ON / OFF
...
{ 
    ...
    // [JN] Отключен баг "Sky never changes"
    ...
    if (testcontrols)
    {
        ...
        players[consoleplayer].message = "ht;bv ghjdthrb eghfdktybz";
    }
} 
	...
	{ 
	  ...
	  case ga_screenshot: 
	    V_ScreenShot("DOOM%02i.%s"); 
			// "screen shot"
            players[consoleplayer].message = DEH_String("cybvjr 'rhfyf");
	    gameaction = ga_nothing; 
	    break; 
	  case ga_nothing: 
	    break; 
	    ...
	    // check for turbo cheats
             ...
             && turbodetected[i])
            {
                ...
                M_snprintf(turbomessage, sizeof(turbomessage),
							// "%s is turbo!"
                           "%s cdth[crjhjcnm!", player_names[i]);
