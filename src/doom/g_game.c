...
{ 
    ...
    p->health_negative = 0;             // [JN] Negative health.
...
static const char *DemoVersionDescription (int version)
{
    ...
    {
        M_snprintf(resultbuf, sizeof(resultbuf), english_language ?
                   "%i.%i (unknown)" : "%i.%i (неизвестная версия)",
...
void G_DoPlayDemo (void)
{
    ...
    if (D_NonVanillaPlayback(demoversion == DOOM_191_VERSION, lumpnum, english_language ?
                             "Doom 1.91 demo format" : "Формат демозаписи Doom 1.91"))
    {
        longtics = true;
    }
