    ...
    switch (joystick_axis->config_stage)
    {
        case CONFIG_CENTER:
            return "Отцентрируйте D-pad или джойстик\n"		// "Center the D-pad or joystick,\n"
                   "и нажмите любую кнопку.";				// "and press a button."

        case CONFIG_STAGE1:
            if (joystick_axis->dir == JOYSTICK_AXIS_VERTICAL)
            {
                return "Нажмите \"вверх\" на джойстике или\n"	// "Push the D-pad or joystick up,\n"
                       "D-pad и нажмите любую кнопку.";			// "and press the button."
            }
            else
            {
                return "Нажмите \"влево\" на джойстике или\n"	// "Push the D-pad or joystick to the\n"
                       "D-pad и нажмите любую кнопку.";			// "left, and press the button."
            }

        case CONFIG_STAGE2:
            if (joystick_axis->dir == JOYSTICK_AXIS_VERTICAL)
            {
                return "Нажмите \"вниз\" на джойстике или\n"	// "Push the D-pad or joystick down,\n"
                       "D-pad и нажмите любую кнопку.";			// "and press the button."
            }
            else
            {
                return "Нажмите \"вправо\" на джойстике или\n"	// "Push the D-pad or joystick to the\n"
                       "D-pad и нажмите любую кнопку.";			// "right, and press the button."
            }
    }
                               ...
                               txt_joystick_axis_callback_t callback)
{
    ...
    {
        TXT_MessageBox(NULL, "Устройство не выбрано!");	// "Please configure a controller first!"
        return;
    }
    ...
    joystick_axis->config_window
        = TXT_NewWindow("Калибровка джойстика/геймпада");	// "Gamepad/Joystick calibration"
...
static void TXT_JoystickAxisDrawer(TXT_UNCAST_ARG(joystick_axis))
{
    ...
    {
        M_StringCopy(buf, "(отсутствует)", sizeof(buf));	// "(none)"
    }
    else if (IS_BUTTON_AXIS(*joystick_axis->axis))
    {
        ...
        M_snprintf(buf, sizeof(buf), "КНОПКИ #%i+#%i", neg, pos);	 // "BUTTONS #%i+#%i"
    }
    else if (IS_HAT_AXIS(*joystick_axis->axis))
    {
        ...
        M_snprintf(buf, sizeof(buf), "ХАТКА #%i (%s)", hat,	// "HAT #%i (%s)"
                   dir == HAT_AXIS_HORIZONTAL ? "horizontal" : "vertical");
    }
    else
    {
        M_snprintf(buf, sizeof(buf), "СТИК #%i", *joystick_axis->axis);	// "AXIS #%i"
    }
