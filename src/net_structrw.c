...
boolean NET_ReadSettings(net_packet_t *packet, net_gamesettings_t *settings)
{
    ...
    for(int i = 0; i < settings->num_players && i < NET_MAXPLAYERS; ++i)
    {
        if(!NET_ReadInt8(packet, (unsigned int *) &settings->player_classes[i]))
        {
            return false;
        }
    }
