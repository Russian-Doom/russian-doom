...
static void UpdateGUI(void)
{
    ...
    {
        startgame = TXT_NewWindowAction(' ', "Начать игру");
...
static void CheckSHA1Sums(void)
{
    ...
    window = TXT_NewWindow("ВНИМАНИЕ!"); // ¬Ќ»ћјЌ»≈!

    cont_button = TXT_NewWindowAction(KEY_ENTER, "Продолжить");
    ...
    {
        TXT_AddWidget(window, TXT_NewLabel
            ("Содежримое папки Ваших WAD-файлами не совпадает с содержимым\n"
...
void NET_WaitForLaunch(void)
{
    ...
    while (net_waiting_for_launch)
    {
        ...
        {
            I_Error("—оединение с сервером потер€но");
        }
