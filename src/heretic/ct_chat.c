...
//
// Описание:
// * Перевод строчек сетевой игры.
// * Добавлена возможность использовать буквы (хъжэбю) в сетевом чате.
// =--------------------------------------------------------------=
                ...
                else if (i == consoleplayer && (*chat_msg[i]))
                {
                    if (numplayers > 1)
                    {
                        P_SetMessage(&players[consoleplayer],
                                     DEH_String("-CJJ<OTYBT JNGHFDKTYJ-"), true);	// -СООБЩЕНИЕ ОТПРАВЛЕНО-
                        S_StartSound(NULL, sfx_chat);
                    }
                    else
                    {
                        P_SetMessage(&players[consoleplayer],
                                     DEH_String("D BUHT YTN LHEUB{ BUHJRJD!"),	 // В ИГРЕ НЕТ ДРУГИХ ИГРОКОВ!
                                     true);
                        S_StartSound(NULL, sfx_chat);
                    }
                }
